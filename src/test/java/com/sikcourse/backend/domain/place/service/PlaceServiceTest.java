...
class PlaceServiceTest {
    ...
    void syncRestaurantsUpdatesExistingPlaceWhenConcurrentInsertCreatesDuplicateContentId() {
        ...
        when(placeUpsertService.upsert(item)).thenThrow(new DuplicatePlaceContentIdException("123456"));
        when(placeUpsertService.updateExisting(item)).thenReturn(PlaceUpsertResult.UPDATED);
        ...
        verify(placeUpsertService).updateExisting(item);
    }
